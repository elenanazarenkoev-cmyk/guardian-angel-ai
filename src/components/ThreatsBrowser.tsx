...
const ThreatsBrowser = ({ userMode }: ThreatsBrowserProps) => {
  ...
  if (isLoading) {
    return (
      <div className="bg-card rounded-2xl p-6 text-center">
        <p className="text-lg text-muted-foreground">Загрузка базы угроз...</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="text-center text-lg text-muted-foreground">
        {userMode === "elderly"
          ? "Изучите типичные схемы мошенников"
          : "Узнай, как обманщики пытаются тебя обмануть! 🕵️"}
      </p>

      <div className="space-y-3">
        {threats?.map((threat) => (
          <div key={threat.id} className="rounded-2xl overflow-hidden">
            <button
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <span className={`inline-block mt-1 px-3 py-1 rounded-full text-xs font-bold ${severityColors[threat.severity] || "bg-muted text-muted-foreground"}`}>
                  {threat.severity === "critical" ? "КРИТИЧЕСКИЙ" : threat.severity === "high" ? "ВЫСОКИЙ" : threat.severity}
                </span>
              </div>
            </button>

            {expandedId === threat.id && (
              <div className="bg-card px-5 pb-5 space-y-3">
                <p className="text-base text-foreground leading-relaxed">{threat.description}</p>

                <div>
                  <p className="text-sm font-bold text-muted-foreground mb-2">🚩 Красные флаги:</p>
                ...
                {threat.examples.length > 0 && (
                  <div>
                    <p className="text-sm font-bold text-muted-foreground mb-2">💬 Примеры фраз:</p>
        ...
        {threats?.length === 0 && (
          <div className="bg-card rounded-2xl p-6 text-center">
            ...
            <p className="text-lg text-muted-foreground">Нет угроз для этой аудитории</p>
          </div>
        )}
      </div>
    </div>
